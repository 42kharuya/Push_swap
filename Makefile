...
SRCS				=	$(LIBFT_FORDER)/ft_atoi.c $(LIBFT_FORDER)/ft_lstnew.c \
							...
							$(PUSH_SWAP_FORDER)/compress.c $(PUSH_SWAP_FORDER)/radix_algo.c \
							...
							$(PUSH_SWAP_FORDER)/sort_2to4.c $(PUSH_SWAP_FORDER)/sort_5to20.c $(PUSH_SWAP_FORDER)/sort_morethan_20.c \
OBJS				= 	$(SRCS:.c=.o)
RM					= 	rm -f
%.o:%.c
	$(CC) $(FLAGS) $(INC) -c $< -o $@
